...
export function MessagesPanel({ messages, disabled }: { messages: MessageRow[]; disabled?: boolean }) {
  ...
  if (messages.length === 0) {
    return (
      <p style={{ fontSize: "15px", color: C.muted, margin: 0, padding: "22px" }}>
    ...
    <div>
      {messages.map((m, i) => (
        <div key={m.id} style={{ borderTop: i === 0 ? "none" : `1px solid ${C.hairline}`, padding: "16px 18px" }}>
          <div style={{ display: "flex", gap: "12px", alignItems: "flex-start", flexWrap: "wrap" }}>
            <div style={{ minWidth: 0, flex: 1 }}>
              <p style={{ fontSize: "15px", fontWeight: 600, color: C.ink, margin: 0 }}>
                ...
                {!m.handled && (
                  <span style={{ fontSize: "11px", fontWeight: 700, color: C.orangeText, backgroundColor: "rgba(250,145,45,.15)", padding: "2px 8px", borderRadius: R.chip, marginLeft: "8px", letterSpacing: "0.06em" }}>
                    NEW
                  </span>
                )}
              </p>
            ...
            <div style={{ display: "flex", gap: "12px", flexShrink: 0 }}>
              ...
              <button
                ...
                style={{ fontFamily: "var(--font-outfit)", fontSize: "13px", fontWeight: 600, color: m.handled ? C.muted : C.greenText, background: "none", border: "none", cursor: disabled ? "not-allowed" : "pointer", minHeight: "40px" }}
              >
          ...
          {open === m.id && (
            <p style={{ fontSize: "15px", color: C.muted, lineHeight: 1.6, margin: "12px 0 0", whiteSpace: "pre-wrap", backgroundColor: C.paper, borderRadius: "12px", padding: "14px 16px" }}>
              {m.message}
            </p>
          )}
        </div>
